    ...
    class ConvertData : ShowData
    {
        ...
        public void convertDataSensor(string data)
        {
            try
            {
                ...
                string hexd = data.Substring(10, 4);    //Gia tri SOt
                ...
                temp = (float)(td * 0.01 - 39.66);                  				//calc. temperature from ticks to [deg Cel]	    -39.6
                //rh_lind = (float)(0.0367 * rhd - 0.0000015955 * rhd * rhd - 2.0468);     	//calc. humidity from ticks to [%RH]for sensor V4
                rh_lind = (float)(0.0405 * rhd - 0.0000028 * rhd * rhd - 4);        //for sensor V3
                humi = (float)(((temp - 25) * (0.01 + 0.00008 * rhd) + rh_lind));   		//calc. temperature compensated humidity [%RH]  -3.7
